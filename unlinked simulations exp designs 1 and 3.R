########################### alternative model with evolution. Experimental Designs 1 and 3 ###################
...
ctpcor<-NULL #phylogenetic correlation coefficients without error corrected by Kelly and Price method
ctrcor<-NULL #raw correlation coefficients without error corrected by Kelly and Price method
ctrslp<-NULL #raw slopes without error corrected by Kelly and Price method
ctpslp<-NULL #phylo slopes without error corrected by Kelly and Price method
...
npcor<-NULL #phylogenetic correlation coefficients with Exp design 3
nrcor<-NULL #raw correlation coefficients with Exp design 3
nrslp<-NULL #raw slopes with Exp design 3
npslp<-NULL #phylo slopes with Exp design 3
...
repeat{
  ...
  # analysis with true phenotypic value
  rn<-ctmaxp-ctmax # calcuate mean reaction norm for each species
  tr.slp<-lm(rn~ctmax)$coefficients[2] # raw slope
  trslp<-c(trslp, as.data.frame(tr.slp)[1,1])
  tp.slp<-lm(pic(rn,tree)~pic(ctmax,tree)+0)$coefficients # pic slope
  tpslp<-c(tpslp, as.data.frame(tp.slp)[1,1])
  talldat<-cbind(as.data.frame(rn), as.data.frame(ctmax))
  names(talldat)<-c("rn", "ctmax")
  tobj<-phyl.vcv(as.matrix(talldat),vcv(tree),1) ## phylogenetic covariance among traits
  tp.cor<-cov2cor(tobj$R)["ctmax","rn"]## phylo correlation between ct & diff
  tpcor<-c(tpcor, tp.cor)
  tr.cor<-cor.test(ctmax, rn, method = "pearson")$estimate # raw correlation
  trcor<-c(trcor, as.data.frame(tr.cor)[1,1])
  
  # Adjusting true values by Kelly and Price Method
  n<-Ntip(tree) # number of tips on the tree
  X<-cbind(ctmax[tree$tip.label],
           ctmaxp[tree$tip.label]) # mean tip values for each trait for each species
  ...
  ctr.slp<-lm(Dstar~ctmax)$coefficients[2] # raw slope corrected rn versus baseline
  ctrslp<-c(crslp, as.data.frame(ctr.slp)[1,1])
  ctp.slp<-lm(pic(Dstar,tree)~pic(ctmax,tree)+0)$coefficients # pic slope with corrected rn values
  ctpslp<-c(ctpslp, as.data.frame(ctp.slp)[1,1])
  ct.alldat<-cbind(as.data.frame(Dstar), as.data.frame(ctmax)[,1]) # corrected data frame for correlation analysis
  names(ct.alldat)<-c("Dstar", "ctmax.hat")
  ct.obj<-phyl.vcv(as.matrix(ct.alldat),vcv(tree),1) ## phylogenetic covariance among traits
  ctp.cor<-cov2cor(ct.obj$R)["Dstar","ctmax.hat"]## phylo correlation between ct & diff corrected
  ctpcor<-c(ctpcor, ctp.cor)
  ctr.cor<-cor.test(ctmax, Dstar, method = "pearson")$estimate # raw correlation
  ctrcor<-c(ctrcor, as.data.frame(ctr.cor)[1,1])
  
  # analysis with phenotypiic values measured with error, Experimental Design 1
...
sumdat<-as.data.frame(cbind(tpcor, tpslp, trcor, trslp, ctpcor, ctpslp, ctrcor, ctrslp, pcor, pslp, rcor, rslp, cpcor, cpslp, crcor, crslp, npcor, npslp, nrcor, nrslp))
